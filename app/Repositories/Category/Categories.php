...
class Categories implements ICategories
{
    ...
    {
        $values=Category::when($request->has("title"),function($q)use($request){
    ...
    function create($data)
    {
        ...
        $data['image']->move(public_path('images\categories'), $imageName);

        $row = new Category();
        $row->title = $data['title'];
        $row->image = 'images\categories\\'.$imageName;
